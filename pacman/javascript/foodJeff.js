...
//
//
Food.prototype.CheckFruit =
    function(tick) {
        if (game.GetCurrentLevelScore() == 70 && !this.fruitPushed) {
            ediblefruit.push(new EdibleFruit('cherry', 'cherry', 12, 17, 200));
            this.fruitPushed = true;
        }


        for (var i = 0; i < ediblefruit.length; i++) {
            ediblefruit[i].Update(tick);
            if (ediblefruit[i].IsPacmanTouched() && ediblefruit[i].CanBeEaten()) {
                //ediblefruit[i].SetCanBeEaten(false);

                pacmans[0].Hide();

                soundPlayer.Play('ghostEaten');
                ediblefruit[i].SetInPointMode();
                game.UpdateScore(ediblefruit[i].GetScore());
                ediblefruit[i].SetCanBeEaten(false);

                setTimeout('ediblefruit[' + i + '].Hide(  );', 500); // ORI CODE ghosts[i].SetInEyeMode( true )
